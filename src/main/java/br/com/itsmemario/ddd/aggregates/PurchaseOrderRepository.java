...
public class PurchaseOrderRepository {
  private Map<String, PurchaseOrder> orders = new HashMap<>();

  public List<PurchaseOrder> findOpenOrdersWithProduct(String product) {
    return orders.values().stream()
        .filter(PurchaseOrder::isOpen)
        .filter(p -> p.containsProduct(product))
        .toList();
  }
